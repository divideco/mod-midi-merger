...
void handle_scheduled_connections(midi_merger_t *const mm) {
  ...
  while ((source = next(mm->ports_to_connect)) != NULL) {
    int result;
    result = jack_connect(mm->client, source->name, jack_port_name(mm->ports[PORT_IN]));
    switch(result) {
    case 0:
      // Fine.
      break;
